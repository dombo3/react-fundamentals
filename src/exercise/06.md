...
- target is form element has 2 descandant dom node first is an input element
  `event.target.elements[0].value`
- element has username as id `event.target.elements.username.value`
- using ref => React was so kind that allowed us to reach the dom node via ref
  value `inputRef.current.value`
- and the currentTarget same as with target but with currentTarget
  `event.currentTarget.elements[0].value`
  `event.currentTarget.elements.username.value`

for event bubbling capturing and target or currentTarget see:
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=06%3A%20Forms&em=dombo.peter%40gmail.com).
